...
* Single click / hold DOWN - Scroll text down (wraps on click)
  * Can be changed in settings to switch to the next notification when already on the bottom instead of wrapping
* Single click / hold UP - Scroll text up (wraps on click)
  * Can be changed in settings to switch to the previous notification when already on the top instead of wrapping
...
In this case, you can select `Show image` action, which will show the image on the watch.

If the image does not fill the watch's screen, you can press SELECT button to zoom in. Then you can press SELECT again to zoom back out.

### Hide from watch

This notification will hide the notification from the watch, while keeping it on the phone. If notification is updated or re-notified on the phone, it will re-appear on the watch.

If you have hidden a notification accidentally, you can bring back all hidden notifications from settings (Double click BACK -> `Restore hidden notifs`).
...
When a rule is created, it does not apply any settings (all of its settings are inherited from above rules by default). Only once you change any setting inside the rule, will that setting start applying.
